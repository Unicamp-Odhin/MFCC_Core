...
module dct_tb ();
  ...
  logic [WIDTH_OUT-1:0] mel_energies[0:NUM_MEL_FILTERS - 1];
  ...
  task dump_mel_energies_to_hex;
    ...
    integer i;
    begin
      fd = $fopen({`TESTS_DIR, "/data/5_energies/0000.hex"}, "w");
  ...
  task dump_coeficients_to_hex;
    ...
    integer i;
    begin
      fd = $fopen({`TESTS_DIR, "/data/6_ceps/0000.hex"}, "w");
