...
module fifo_tb;
  ...
  initial begin
    ...
    for (int i = 0; i < DEPTH; i++) begin
      if (i < DEPTH - 1) begin
        assert (!full_o)
        else $error("Erro: FIFO não deveria estar cheia! i: %0d", i);
      end else begin
        // Último elemento, a FIFO deve estar cheia
        assert (full_o)
        else $error("Erro: FIFO deveria estar cheia!");
      end
    ...
    wr_en_i = 0;
    assert (full_o)
    else $error("Erro: FIFO deveria continuar cheia!");

    // Ler até esvaziar
    for (int i = 0; i < DEPTH; i++) begin
      if (i < DEPTH - 1) begin
        assert (!empty_o)
        else $error("Erro: FIFO não deveria estar vazia! i: %0d", i);
      end else begin
        // Último elemento, a FIFO deve estar vazia
        assert (empty_o)
        else $error("Erro: FIFO deveria estar vazia!");
      end
      ...
      if (i < DEPTH - 1) begin
        assert (read_data_o == i)
        else $error("Erro: Dados lidos incorretamente! i: %0d", i);
      end
    ...
    rd_en_i = 0;
    assert (empty_o)
    else $error("Erro: FIFO deveria continuar vazia!");
