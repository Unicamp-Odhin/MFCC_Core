...
C_DUMP_DIR = os.path.join(REF_C_DIR, 'dumps', '4_power_spectrum')
PY_DUMP_DIR = os.path.join(REF_PYTHON_DIR, 'dumps', '4_power_spectrum')
...
def main():
    ...
    cabecalho = ['F_FFT', 'Erro Absoluto Médio', 'Erro Absoluto Máximo',
                 'Erro Relativo Médio', 'Erro Quadrático Médio']
    resultados = []

    # Loop sobre F_FFT (0 a 30)
    for F_FFT_local in range(0, 30):
        global F_FFT
        F_FFT = F_FFT_local
        write_config()
        ...
        try:
            ...
            print(f"  Erro ao executar C para F_FFT={F_FFT}: {e}")
            continue
        ...
        if c_data is None or len(c_data) == 0:
            print(f"  Nenhum dump encontrado para F_FFT={F_FFT} em {C_DUMP_DIR}")
            continue

        # Alinha tamanhos (usa o menor)
        min_len = min(len(c_data), len(py_data))
        if min_len == 0:
            print(f"  Dados vazios para F_FFT={F_FFT}")
            continue
    ...
    # Salva CSV
    with open('metrics_fft.csv', 'w', newline='') as f:
        writer = csv.writer(f, delimiter=',')
        writer.writerow(cabecalho)
        writer.writerows(resultados)

    print("\nResultados salvos em metrics_fft.csv")
