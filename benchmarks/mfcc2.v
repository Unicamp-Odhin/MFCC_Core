...
module dct (
	...
	(* ramstyle = "logic" *) reg signed [INPUT_WIDTH - 1:0] mel_filters [0:NUM_FILTERS - 1];
...
module mel (
	...
	reg [130:0] stage_load;
	reg [130:0] stage_mul;
	reg [130:0] stage_sum;
	reg [130:0] stage_energy;
	wire [10:0] i_total_next;
	wire [7:0] temp_log2;
	always @(posedge clk)
		if (!rst_n) begin
			stage_load <= 1'sb0;
			...
			if (mel_start_i || (stage_energy[37-:6] < NUM_FILTERS)) begin
				stage_load[37-:6] <= stage_energy[37-:6];
				stage_load[66-:11] <= stage_energy[66-:11];
				stage_load[55-:9] <= mel_memory[stage_energy[66-:11]][8:0];
				stage_load[46-:9] <= mel_memory[stage_energy[66-:11] + 1][8:0];
				stage_load[31-:32] <= 0;
			end
			stage_mul <= stage_load;
			stage_mul[130-:32] <= power_spectrum_mem[stage_load[55-:9]];
			stage_mul[98-:32] <= mel_memory[((stage_load[66-:11] + 2) + stage_load[55-:9]) - stage_load[55-:9]][31:0];
			stage_sum <= stage_mul;
			stage_sum[31-:32] <= stage_mul[31-:32] + (((stage_mul[130-:32] * stage_mul[98-:32]) + 1073741824) >> 31);
			stage_sum[55-:9] <= stage_mul[55-:9] + 1;
			stage_energy <= stage_sum;
			if (stage_sum[55-:9] > stage_sum[46-:9]) begin
				mel_valid <= 1;
				mel_prt_energies <= stage_sum[37-:6];
				if (stage_sum[31-:32] <= 0)
					mel_value_energies <= 8'h00;
				else
					mel_value_energies <= temp_log2;
				stage_energy[66-:11] <= stage_sum[66-:11] + 33;
				stage_energy[37-:6] <= stage_sum[37-:6] + 1;
			end
			else
				mel_valid <= 0;
			if (stage_energy[37-:6] >= NUM_FILTERS)
				mel_done_o <= 1;
			else
				mel_done_o <= 0;
		end
	base2log u_base2log(
		.number_i(stage_energy[31-:32]),
		.log_o(temp_log2)
	);
endmodule
module MFCC_Core (
