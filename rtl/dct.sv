...
module dct #(
  ...
  always_comb begin
    ...
    case (dct_current_state)
      ...
      IDLE: begin
        if (start_i) dct_next_state = PROCESSING;
      end

      PROCESSING: begin
        if (n_ptr == NUM_MEL_FILTERS) dct_next_state = UPDATE_K;
      end

      UPDATE_K: begin
        if (k_ptr == NUM_CEPS - 1) dct_next_state = DONE;
        else dct_next_state = PROCESSING;
      end
